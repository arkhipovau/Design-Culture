...
const heroRe =
  /(<section class="o-interview-hero a-section"[^>]*>[\s\S]*?<p class="o-interview-hero__meta">[^<]*<\/p>\s*<\/div>)(\s*<\/section>)/;

for (const file of fs.readdirSync(dir).filter((f) => f.endsWith('.html'))) {
  const p = path.join(dir, file);
  let html = fs.readFileSync(p, 'utf8');

  if (html.includes('class="m-interview-portrait"')) {
    console.log('skip:', file);
    continue;
  }

  const authorMatch = html.match(/<p class="o-interview-hero__author">([^<]*)<\/p>/);
  ...
  try {
    ...
    const inline = committed.match(
      /<figure class="m-interview-portrait">[\s\S]*?<img src="([^"]+)"/
    );
